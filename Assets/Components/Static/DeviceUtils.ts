/*
 * DeviceUtils
 * Manage the different device behaviours. 
 * It's a static class and can be used without instantiating.
 */

export default class DeviceUtils {

    static clickEventName = DeviceUtils.isMobile() ? "touchstart" : "click";

    static rtime: Date;
    static timeout: boolean = false;
    static delta: number = 600;

    static onResizeCompleteCb: (() => void)[] = [];        // Array of callbacks to fire when window size has changed
    ...
    static getClickEventName() {
        return DeviceUtils.isMobile() ? "touchstart" : "click";
    }
